...
def get_fields_collisions(particles: Particles) -> list[tuple[float, float, float]]:
    ''' calculates the electric field generated by each particle within our system with the addition of a weak field so that we can model collisions'''
    output = []
    n = particles.size() #length of the particles
    temp = [[0, 0, 0] for _ in range(n)]
    i = 0 #index1
    while(i < n): # for every particle within the group of particles we give
        j = (i + 1) % n #index2
        curr = temp[i]
        pos1, q1, _ = particles.at(i) # position of first particle
        while(j != i):
            pos2, q, _ = particles.at(j) # position of other particle
            vect = [pos1[0] - pos2[0], pos1[1] - pos2[1], pos1[2] - pos2[2]]
            distant = ((vect[0] ** 2) + (vect[1] ** 2) + (vect[2] ** 2)) ** 0.5 # distance between the particles
