...
def find_pos_vel(velocity, fields, mass, charge): # input is the velocities @ t0 (beginning of step, and mass of the particle
    ...
    Force_B_ext=charge*np.cross(velocity, fields[2]) # F=q*(v×B)
...
'''Isabella: take a look at how I did this, I basically made an arbitrary weak force, and then because the output has to be a field I multiplied it by q?
    Wasn't sure the best way to do this. Also we want to think about what value to use for if distant<=____ (3 was arbitrary)'''

# get_fields if we're incorporating weak force/collisions
def get_fields_collisions(particles: Particles) -> list[tuple[float, float, float]]:
    ''' calculates the electric field generated by each particle within our system '''
    output = []
    n = particles.size() #length of the particles
    temp = [[0, 0, 0] for _ in range(n)]
    ...
    while(i < n): # for every particle within the group of particles we give
        ...
        while(j != i):
            ...
            if distant<=3: # for small enough distances between the particles (determine exact number later)
                ...
                weak_force = (1/math.log(y[i]), 1/math.log(y[i+1]), 1/math.log(y[i+2])) # test force, arbitrary
                weak_field = weak_force * q

                curr[0] += (unit_vect[0] * e_mag) + weak_field[0] # E_x with weak
                curr[1] += (unit_vect[1] * e_mag) + weak_field[1] # E_y with weak
                curr[2] += (unit_vect[2] * e_mag) + weak_field[2] # E_z with weak   
 ...
 # get function (grabs fields, positions, etc)   
