...
def get_fields2(particles: Particles) -> list[tuple[float, float, float]]:
    ...
    while(i < n): # for every particle within the group of particles we give
        ...
        i += 1
    for field in temp:
        output.append((field[0], field[1], field[2])) # for each particle output the electric field
    return output 
...
def get_function(particles, get_fields):
    ...
    def find_pos_vel(t: float, y: list[float], ext_field): #y: [pos_x1, pos_y1, post_z1, vel_x1, vel_y1, vel_z1, pos_x2, pos_y2....]
        ...
        while(i < n):
            ...
            output.append(y[i + 5])
            '''
            Isabella: Adding in external magnetic fields in as well
            '''  
            # external electric field:
            ext_x, ext_y, ext_z = ext_field(*pos)
            e_x, e_y, e_z = get_function.fields[particle_index]
            e_x += ext_x # adding external and point fields together in 3D
            e_y = ext_y
            e_z += ext_z
            ...
            '''^^Isabella: we need to make sure to incorporate this into the input eventually'''
            ...
            ''' Isabella:
            Have to figure out how to grab the velocities (eventually replace the variable velocity)
            '''
